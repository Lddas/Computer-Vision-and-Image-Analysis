...
def extract_patches(img, p):
    ...
    '''H, W, C = img.shape
    patches =  np.empty((H, W, 9, 3))
    left = np.roll(img, 1, axis = 1)
    right = np.roll(img, -1, axis = 1)
    down = np.roll(img, -1, axis = 0)
    top = np.roll(img, 1, axis = 0)
    top_left = np.roll(left, 1, axis = 0)
    top_right = np.roll(right, 1, axis = 0)
    down_left = np.roll(left, -1, axis = 0)
    down_right = np.roll(right, -1, axis = 0)

    for i in range(H):
      for j in range(W):
          patches[i][j][0] = top_left[i][j]
          patches[i][j][1] = top[i][j]
          patches[i][j][2] = top_right[i][j]
          patches[i][j][3] = left[i][j]
          patches[i][j][4] = img[i][j]
          patches[i][j][5] = right[i][j]
          patches[i][j][6] = down_left[i][j]
          patches[i][j][7] = down[i][j]
          patches[i][j][8] = down_right[i][j]
  '''
